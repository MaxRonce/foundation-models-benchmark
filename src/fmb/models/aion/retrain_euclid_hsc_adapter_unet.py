...
def parse_args() -> argparse.Namespace:
    ...
    # Data
    parser.add_argument("--cache-dir", type=str, default=defaults.get("cache_dir", str(paths.dataset)))
    parser.add_argument("--split", type=str, default=defaults.get("split", "train"))
    parser.add_argument("--max-entries", type=int, default=defaults.get("max_entries", 0))
    ...
    parser.add_argument("--lr", type=float, default=float(defaults.get("learning_rate", 1e-4)))
    parser.add_argument("--grad-clip", type=float, default=defaults.get("grad_clip", 1.0))
    parser.add_argument("--accum-steps", type=int, default=defaults.get("accum_steps", 1))
    parser.add_argument("--amp-dtype", type=str, default=defaults.get("amp_dtype", "float16"), choices=["float16", "bfloat16"])
    ...
    parser.add_argument("--cpu-crop", action="store_true", default=defaults.get("cpu_crop", False))
    
    # Model
    parser.add_argument("--hidden", type=int, default=defaults.get("hidden", 16))
    parser.add_argument("--use-unet-checkpointing", action="store_true", default=defaults.get("use_unet_checkpointing", False))
    parser.add_argument("--codec-grad", type=str, default=defaults.get("codec_grad", "ste"), choices=["ste", "full"])
    parser.add_argument("--disable-codec-checkpointing", action="store_true", default=defaults.get("disable_codec_checkpointing", False))
    
    # Output
    default_out = paths.retrained_weights / "aion"
    parser.add_argument("--output", type=str, default=defaults.get("out_dir", str(default_out)))
    parser.add_argument("--resume-adapter", type=str, default=defaults.get("resume_adapter", None))
    parser.add_argument("--auto-resume", action="store_true", default=defaults.get("auto_resume", True))
    
    # Logging
    parser.add_argument("--num-workers", type=int, default=defaults.get("num_workers", 0))
    parser.add_argument("--log-gpu-mem-every", type=int, default=defaults.get("log_gpu_mem_every", 50))
        ...
        for step, euclid_img in enumerate(progress):
            ...
            if (step + 1) % args.accum_steps == 0:
                if args.grad_clip > 0:
                    ...
                    nn.utils.clip_grad_norm_(list(euclid_to_hsc.parameters()) + list(hsc_to_euclid.parameters()), args.grad_clip)
            ...
            if device.type == "cuda" and (step + 1) % args.log_gpu_mem_every == 0:
                ...
                progress.set_postfix(loss=f"{loss_val:.4f}", mem=f"{alloc:.1f}/{peak:.1f}G")
            else:
