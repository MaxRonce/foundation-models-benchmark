...
def plot_panel(ax, source_name, target_name, source_ranks, target_ranks, n_total, color):
    ...
    # Plot Histogram
    ax.hist(ranks_pct, bins=40, range=(0, 100), color=color, edgecolor='black', alpha=0.7, linewidth=0.5, zorder=3)
    ...
    # Calculate stats
    retained_1 = (target_ranks[subset_mask] <= top_1_threshold).sum() / subset_mask.sum() * 100
    retained_10 = (target_ranks[subset_mask] <= (n_total * 0.1)).sum() / subset_mask.sum() * 100
    ...
    ax.set_title(rf"\textbf{{{source_name}}} $\rightarrow$ \textbf{{{target_name}}}", pad=5)
    ax.grid(True, linestyle=':', alpha=0.4, zorder=0)

def main(argv: Sequence[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Generate multi-model/multi-type displacement analysis grid")
    ...
    parser.add_argument("--save", default="paper_displacement_grid.png", help="Output filename")
