...
def run_analysis(
    ...
    if big_merged is None or big_merged.empty:
        print("Error: No common objects found after merge. Check object_ids consistency.")
        return
    ...
    for col1 in final_cols:
        for col2 in final_cols:
            s1 = top_1_sets[col1]
            s2 = top_1_sets[col2]
            iou = len(s1.intersection(s2)) / len(s1.union(s2)) if len(s1.union(s2)) > 0 else 0
            jaccard_matrix.loc[col1, col2] = iou

    plt.figure()
    g2 = sns.clustermap(jaccard_matrix, annot=True, fmt=".2f", cmap="YlGnBu", 
                        dendrogram_ratio=(.1, .1)) #, cbar_pos=(.02, .32, .03, .2))
    g2.fig.suptitle(r"Hierarchical Clustering of Top 1\% Anomalies (Jaccard Index)", y=1.02)
