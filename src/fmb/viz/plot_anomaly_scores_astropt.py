...
Adapted for AstroPT embeddings.

Usage:
    python -m scratch.plot_anomaly_scores_astropt \
        --embeddings /path/to/astropt_embeddings.pt \
        --scores-csv scratch/outputs/anomaly_scores_astropt.csv \
        --output-dir scratch/outputs/anomaly_umaps_astropt
"""
import argparse
import csv
...
# Import from the AstroPT version to get correct keys and stacking logic
from .detect_outliers_astropt import EMBEDDING_KEYS, load_records, stack_embeddings  # type: ignore[import]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot UMAP projections of embeddings coloured by flow-based anomaly scores (AstroPT version).",
    )
    parser.add_argument("--embeddings", required=True, help="Path to embeddings .pt file")
    parser.add_argument("--scores-csv", required=True, help="CSV produced by scratch.detect_outliers_NFs_astropt")
