...
Now uses an autoregressive Neural Spline Flow (NSF) with LULinearPermute.
...
def build_flow(
    ...
    """
    Build an autoregressive Neural Spline Flow (NSF) with LULinearPermute.

    - Base distribution: Diagonal Gaussian in R^dim
    - Each transform: Autoregressive RQ-spline + LULinearPermute
    """
    base = nf.distributions.base.DiagGaussian(dim)
    flows: list[nn.Module] = []

    # Hyperparams internal to the NSF blocks; can be exposed via CLI later if needed
    hidden_layers = 2          # number of hidden layers in the autoregressive net
    num_bins = 8               # spline bins
    tail_bound = 3.0           # spline support

    for _ in range(num_transforms):
        # Autoregressive Rational Quadratic Spline transform
        flows.append(
        ...
        # Invertible linear mixing (Glow-style) instead of simple permutation
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a normalizing-flow density model on embeddings and compute anomaly scores (AstroPT version, NSF autoregressive).",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=6,
        help="Number of NSF blocks (AutoregressiveRationalQuadraticSpline + LULinearPermute) in the flow",
    )
    parser.add_argument(
        "--hidden-features",
        type=int,
        default=256,
        help="Hidden width in the autoregressive spline networks",
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    for key in requested_keys:
        ...
        print(
            f"[{key}] training NSF-autoregressive flow with dim={embeddings_tensor.shape[1]}, "
