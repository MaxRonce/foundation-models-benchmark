...
KNOWN_KEY_PAIRS = {
    ...
    "aion": ("embedding_hsc", "embedding_spectrum"), # heuristic, might be embedding_hsc_desi
...
def process_file(
    ...
    for i, rec in enumerate(records):
        ...
        def get_np(k):
            ...
            if isinstance(v, torch.Tensor): return v.detach().cpu().numpy().flatten()
    ...
    with open(output_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["object_id", "cosine_similarity", "rank"])
        writer.writeheader()
...
def main(argv: List[str] = None):
    parser = argparse.ArgumentParser(description="Compute Cosine Similarity (Image vs Spectrum)")
    # Optional overrides
    ...
    # AION
    p = Path(args.aion_embeddings) if args.aion_embeddings else (paths.embeddings / "aions_embeddings.pt")
    if p.exists(): inputs.append((p, "aion"))
    
    # AstroPT
    p = Path(args.astropt_embeddings) if args.astropt_embeddings else (paths.embeddings / "astropt_embeddings.pt")
    if p.exists(): inputs.append((p, "astropt"))
    
    # AstroCLIP
    p = Path(args.astroclip_embeddings) if args.astroclip_embeddings else (paths.embeddings / "embeddings_astroclip.pt")
    if p.exists(): inputs.append((p, "astroclip"))
